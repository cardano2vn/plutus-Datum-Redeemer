...
[![Mã trong Cardano Workspace](https://demeter.run/code/badge.svg)](https://demeter.run/code?repository=https://github.com/cardano2vn/plutus-Datum-Redeemer.git&template=plutus)
...
Trình xác nhận rất cơ bản của chúng tôi có một nhiệm vụ đơn giản: đảm bảo rằng giá trị số nguyên trong Datum bằng giá trị số nguyên trong Redeemer  (khá ngớ ngẩn và vô dụng phải không).

### 1. Tạo mã hash cho Datum

**Khi khóa tiền**

Ví dụ ta có thể chuyển vào Datum một giá trị bất kỳ ở đây ta dùng số nguyên 42 và chuyển thành hàm băm của giá trị đó. Để làm được điều nay, chúng ta cần một biểu diễn JSON của Datum để cardano-cli chuyển nó thành hàm băm. Tệp `assets/lock.datum` chứa một ví dụ về biểu diễn JSON cho một Datum chứa giá trị `42`:
...
Để mở khóa Tài sản (tADA), chúng ta cần chuẩn bị một giao dịch sử dụng UTxO từ địa chỉ hợp đồng và bao gồm cả `Redeemer` và tuân thủ các điều kiện do trình xác thực của chúng ta xác định. Với hợp đồng luôn luôn đúng này thì cho bất kỳ giá trị `Redeemer` nào vào cũng được.
